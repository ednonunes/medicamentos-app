<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-xl text-emerald-800 leading-tight">
            {{ __('Editar Medicamento') }}: {{ $medication->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl p-6 border border-gray-100">
                ...
                <form action="{{ route('medications.update', $medication->id) }}" method="POST" class="space-y-4" onsubmit="showLoadingForm(this)">
                    @csrf 
                    @method('PUT')
    ...
    <script>
        function showLoadingForm(form) {
            ...
            // Trava o botão para impedir requisições duplicadas no servidor
            btn.disabled = true;
            
            // Exibe o feedback visual de rotação
